...
export const getStores = http => userId =>
  http.get(`https://buxy-proxy.herokuapp.com/stores/userId/${userId}`, userId)

export const getStoresTask = http => userId =>
  new Task((rej, res) => getStores(http)(userId).then(res, rej))

export const loadTask = http => compose(
  map(map(addTerms)),
  map(map(toViewModel)),
  chain(eitherToTask),
  map(parse),
